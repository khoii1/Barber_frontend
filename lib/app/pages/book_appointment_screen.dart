...
import '../../data/datasources/remote/availability_service.dart';
...
class BookAppointmentScreen extends StatefulWidget {
  final List<Service> services; // Nhận danh sách dịch vụ
...
class _BookAppointmentScreenState extends State<BookAppointmentScreen> {
  ...
  Set<String> _availableStylistIds =
      {}; // IDs của các stylists có schedule vào ngày được chọn
  ...
  Future<void> _loadAvailableStylistsForDate(DateTime date) async {
    ...
    try {
      ...
      final schedules = await StylistScheduleService.getSchedulesByDate(
        dateStr,
      );
      ...
      for (final schedule in schedules) {
        ...
        print(
          '🔍 Processing schedule: ${schedule['_id']}, stylistId type: ${stylistId.runtimeType}',
        );
        ...
        if (id != null && id.isNotEmpty) {
          ...
          print(
            '⚠️ Invalid stylistId in schedule: $stylistId (type: ${stylistId.runtimeType})',
          );
        }
      }
      ...
      if (mounted) {
        ...
        print(
          '📋 All stylists in provider: ${stylistProvider.stylists.map((s) => s.id).toList()}',
        );

        // Kiểm tra xem có stylist nào match không
        // Normalize IDs để so sánh (loại bỏ khoảng trắng, chuyển về lowercase)
        final normalizedAvailableIds = availableIds
            .map((id) => id.trim().toLowerCase())
            .toSet();
        final matchingStylists = stylistProvider.stylists.where((s) {
        ...
        print(
          '🎯 Matching stylists: ${matchingStylists.map((s) => s.fullName).toList()}',
        );
        ...
        final isSelectedStylistAvailable =
  ...
  Future<void> _loadAvailableSlots() async {
    ...
    try {
      // Sử dụng dịch vụ đầu tiên để lấy available slots
      // (có thể tính toán dựa trên tổng thời gian nếu cần)
      final slots = await AvailabilityService.getAvailableSlots(
        stylistId: _selectedStylist!.id,
        serviceId: widget.services.first.id,
        date: _selectedDate,
      );
  ...
  Future<void> _submitBooking() async {
    ...
    // Tạo appointment cho từng dịch vụ
    bool allSuccess = true;

    for (final service in widget.services) {
      // Tính thời gian bắt đầu cho từng dịch vụ
      DateTime serviceStartAt = startAt;
      if (widget.services.indexOf(service) > 0) {
        // Dịch vụ tiếp theo bắt đầu sau khi dịch vụ trước kết thúc
        int previousDuration = 0;
        for (int i = 0; i < widget.services.indexOf(service); i++) {
          previousDuration += widget.services[i].durationMin;
        }
        serviceStartAt = startAt.add(Duration(minutes: previousDuration));
      }
...
class _HeaderSection extends StatelessWidget {
  ...
  List<DateTime> _getDateOptions() {
    ...
    for (int i = 0; i < 14; i++) {
      dates.add(
        DateTime(todayLocal.year, todayLocal.month, todayLocal.day + i),
      );
    }
    return dates;
  }
    ...
    showModalBottomSheet(
      ...
      builder: (context) => Consumer<StylistProvider>(
        builder: (context, stylistProvider, _) {
          ...
          final normalizedAvailableIds = availableStylistIds
              .map((id) => id.trim().toLowerCase())
              .toSet();
          final availableStylists = stylistProvider.stylists
              .where(
                (s) =>
                    normalizedAvailableIds.contains(s.id.trim().toLowerCase()),
              )
              .toList();
                ...
                if (isLoadingStylists)
                  ...
                  ...availableStylists.map(
                    (s) => ListTile(
                      ...
                      subtitle: Text(
                        'Rating: ${s.ratingAvg.toStringAsFixed(1)}',
                      ),
